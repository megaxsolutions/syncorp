...
export default function SssEeShare() {
  ...
  // Fetch employees and SSS contributions on component mount
  useEffect(() => {
    fetchEmployees();
    fetchContributions();
  }, []);
  ...
  // Fetch all SSS contributions
  const fetchContributions = async () => {
    setLoading(true);
    try {
      const response = await axios.get(
        `${config.API_BASE_URL}/sss_contributions/get_all_sss_contribution`,
        {
          headers: {
            "X-JWT-TOKEN": localStorage.getItem("X-JWT-TOKEN"),
            "X-EMP-ID": localStorage.getItem("X-EMP-ID"),
      ...
      console.error("Fetch SSS contributions error:", error);
      Swal.fire({
        icon: "error",
        title: "Error",
        text: "Failed to load SSS contribution data.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const addContribution = async (e) => {
    ...
    try {
      ...
      // Send request to create SSS contribution
      const response = await axios.post(
        `${config.API_BASE_URL}/sss_contributions/add_sss_contribution`,
        formData,
        {
          headers: {
            "X-JWT-TOKEN": localStorage.getItem("X-JWT-TOKEN"),
      ...
      Swal.fire({
        icon: "success",
        title: "Success",
        text: response.data.success || "SSS contribution added successfully.",
        timer: 1500,
      ...
      console.error("Add SSS Contribution Error:", error);
      Swal.fire({
        icon: "error",
        title: "Error",
        text: error.response?.data?.error || "Failed to add SSS contribution.",
      });
    }
  };

  // Open edit modal for an SSS contribution
  const openEditModal = (contribution) => {
    Swal.fire({
      title: "Edit SSS Employee Share",
      html: `
  ...
  // Update an SSS contribution
  const updateContribution = async (id, updatedData) => {
    try {
      const response = await axios.put(
        `${config.API_BASE_URL}/sss_contributions/update_sss_contribution/${id}`,
        updatedData,
        {
          headers: {
            "X-JWT-TOKEN": localStorage.getItem("X-JWT-TOKEN"),
      ...
      Swal.fire({
        icon: "success",
        title: "Success",
        text: response.data.success || "SSS contribution updated successfully.",
        timer: 1500,
        showConfirmButton: false
      });

      // Refresh SSS contributions data
      fetchContributions();
    } catch (error) {
      console.error("Update SSS Contribution Error:", error);
      Swal.fire({
        icon: "error",
        title: "Error",
        text: error.response?.data?.error || "Failed to update SSS contribution.",
      });
    }
  };

  // Delete an SSS contribution
  const handleDelete = (contribution) => {
    Swal.fire({
      title: "Are you sure?",
      text: `Do you want to delete the SSS contribution for ${contribution.fullname}?`,
      icon: "warning",
      showCancelButton: true,
      ...
      if (result.isConfirmed) {
        try {
          const response = await axios.delete(
            `${config.API_BASE_URL}/sss_contributions/delete_sss_contribution/${contribution.id}`,
            {
              headers: {
                "X-JWT-TOKEN": localStorage.getItem("X-JWT-TOKEN"),
                "X-EMP-ID": localStorage.getItem("X-EMP-ID"),
          ...
          Swal.fire({
            icon: "success",
            title: "Success",
            text: response.data.success || "SSS contribution has been deleted.",
            timer: 1500,
            showConfirmButton: false
          });
          ...
          console.error("Delete SSS Contribution Error:", error);
          Swal.fire({
            icon: "error",
            title: "Error",
            text: error.response?.data?.error || "Failed to delete SSS contribution.",
          });
        }
      }
    });
    ...
    <>
      ...
      <div id="main" className="main">
        <div className="pagetitle mb-4">
          <h1>SSS Employee Share Management</h1>
        ...
        <div className="row">
          {/* Left column: Add SSS EE Share Form */}
          <div className="col-md-5 mb-4">
            <div className="card shadow-sm">
              <div className="card-header bg-primary text-white">
                <h2 className="h5 mb-0">Add SSS Employee Share</h2>
              </div>
                ...
                <form onSubmit={addContribution}>
                  ...
                  <div className="mb-3">
                    ...
                    <small className="text-muted">
                      Enter the employee's contribution amount for SSS
                    </small>
                  </div>

                  <div className="d-grid mt-4">
                    <button type="submit" className="btn btn-primary">
                      <i className="bi bi-plus-circle me-2"></i>Add SSS Employee Share
                    </button>
                  </div>
                </form>
              </div>
          ...
          {/* Right column: SSS EE Shares List */}
              ...
              <div className="card-body">
                {loading ? (
                  <div className="text-center py-4">
                    ...
                    <p className="mt-2">Loading SSS employee shares...</p>
                  </div>
                ) : contributions.length === 0 ? (
                  <div className="alert alert-info">
                    <i className="bi bi-info-circle me-2"></i>
                    No SSS employee shares recorded yet.
                  </div>
                ) : (
                  <>
