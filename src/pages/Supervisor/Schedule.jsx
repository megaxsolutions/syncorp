...
const SupervisorSchedule = () => {
  ...
  const [autoOffPeriod, setAutoOffPeriod] = useState("thisMonth") // 'thisMonth', 'nextMonth', 'custom'
  const [autoOffStartMonth, setAutoOffStartMonth] = useState(moment().format("YYYY-MM"))
  const [autoOffEndMonth, setAutoOffEndMonth] = useState(moment().add(2, "months").format("YYYY-MM"))
  // Add this state for custom date selection
  const [customOffDates, setCustomOffDates] = useState([])
  ...
  // Update the fetchBreakSchedules function to pass supervisor ID
  ...
  // Update the fetchSchedules function to use supervisor-specific endpoints
  ...
  const fetchSchedules = async () => {
    try {
      ...
      const [regularResponse, overtimeResponse] = await Promise.all([
        axios.get(`${config.API_BASE_URL}/shift_schedules/get_shift_schedule_day_supervisor/${supervisorId}`, {
          headers: {
            "X-JWT-TOKEN": localStorage.getItem("X-JWT-TOKEN"),
            "X-EMP-ID": supervisorId,
          },
        }),
        axios.get(`${config.API_BASE_URL}/shift_schedules/get_shift_schedule_day_overtime_supervisor/${supervisorId}`, {
          headers: {
            "X-JWT-TOKEN": localStorage.getItem("X-JWT-TOKEN"),
            "X-EMP-ID": supervisorId,
          },
  ...
  const handleDeleteSchedule = async (empId, day, scheduleType, isOvertime) => {
    try {
      ...
      if (result.isConfirmed) {
        ...
        const response = await axios.delete(`${config.API_BASE_URL}/shift_schedules/${endpoint}/${scheduleType}`, {
          headers: {
            "X-JWT-TOKEN": localStorage.getItem("X-JWT-TOKEN"),
            "X-EMP-ID": localStorage.getItem("X-EMP-ID"),
          },
  ...
  // Update handleBulkDeleteSubmit function to properly handle off days
    ...
    try {
      ...
      if (result.isConfirmed) {
        ...
        if (deleteScheduleType === "both" || deleteScheduleType === "regular") {
          // Delete regular schedules (type 1)
          ...
          // Also delete off days schedules (type 0) when deleting regular schedules
          const offDaysPromise = axios.delete(
            `${config.API_BASE_URL}/shift_schedules/delete_shift_schedule_multiple_day/0`,
            {
              headers: {
                "X-JWT-TOKEN": localStorage.getItem("X-JWT-TOKEN"),
                "X-EMP-ID": localStorage.getItem("X-EMP-ID"),
          ...
          deletePromises.push(offDaysPromise)
        }
  ...
  // 4. Update the getSchedulesForDay function to include breaks and off days
  const getSchedulesForDay = (day) => {
    if (!day) return { regular: [], overtime: [], breaks: [], offs: [] }

    const dateStr = moment(currentDate).date(day).format("YYYY-MM-DD")

    const regular = regularSchedules.filter(
      (schedule) => moment(schedule.day).format("YYYY-MM-DD") === dateStr && schedule.schedule_type !== 0, // Exclude "off" type schedules from regular
    )

    const overtime = overtimeSchedules.filter((schedule) => moment(schedule.day).format("YYYY-MM-DD") === dateStr)

    const breaks = breakSchedules.filter((schedule) => moment(schedule.day).format("YYYY-MM-DD") === dateStr)

    // Add a separate category for "off" days (schedule_type=0)
    const offs = regularSchedules.filter(
      (schedule) => moment(schedule.day).format("YYYY-MM-DD") === dateStr && schedule.schedule_type === 0,
    )

    return { regular, overtime, breaks, offs }
  }
  ...
  // Update the handleAutoOffWeekends function
  const handleAutoOffWeekends = () => {
    if (selectedEmployees.length === 0) {
      Swal.fire({
        icon: "warning",
        title: "No Employees Selected",
    ...
    setAutoOffPeriod("thisMonth") // Reset to default
    setAutoOffStartMonth(moment().format("YYYY-MM"))
    setAutoOffEndMonth(moment().add(2, "months").format("YYYY-MM"))
    setCustomOffDates([]) // Clear custom dates
  ...
  const handleSearch = () => {
    if (searchSelection) {
      ...
      const filtered = employees.filter((emp) => emp.emp_ID === searchSelection.value)
  ...
  // Add handleAutoOffSubmit function
  const handleAutoOffSubmit = async () => {
    try {
      // Get weekend dates based on selected period
      const weekendDates = []
      let startDate, endDate

      if (autoOffPeriod === "thisMonth") {
        startDate = moment().startOf("month")
        endDate = moment().endOf("month")
      } else if (autoOffPeriod === "nextMonth") {
        startDate = moment().add(1, "month").startOf("month")
        endDate = moment().add(1, "month").endOf("month")
      } else if (autoOffPeriod === "custom") {
        startDate = moment(autoOffStartMonth + "-01")
        endDate = moment(autoOffEndMonth + "-01").endOf("month")

        // Add any specific custom dates selected
        if (customOffDates.length > 0) {
      ...
      if (startDate.isAfter(endDate)) {
        Swal.fire({
          ...
          text: "Start month must be before end month",
        })
        return
      }

      // Find all weekends (Saturday and Sunday) in the range
      ...
      if (weekendDates.length === 0) {
        Swal.fire({
          ...
          text: "No weekend days or custom dates found in the selected period",
        })
        return
      }
      ...
      Swal.fire({
        title: "Setting weekend schedules...",
        allowOutsideClick: false,
        didOpen: () => {
          Swal.showLoading()
        },
      ...
      // Use the API endpoint for setting schedules with schedule_type_id = 0 (for off days)
      // Modify this section in handleAutoOffSubmit
      const requestData = {
        array_employee_emp_id: selectedEmployees,
        admin_emp_id: localStorage.getItem("X-EMP-ID"),
        shift_in: shiftInTime,
        shift_out: shiftOutTime, // Using the user-defined shift times
        array_selected_days: weekendDates,
        schedule_type_id: 0, // Change from 2 to 0 for "off" days
      }
      const response = await axios.post(
        `${config.API_BASE_URL}/shift_schedules/add_shift_schedule_multiple_day`,
        requestData,
      ...
      Swal.fire({
        icon: "success",
        title: "Success",
        text: `${weekendDates.length} days have been set as off days`,
        timer: 2000,
        showConfirmButton: false,
      })
      ...
      console.error("Error setting weekend off days:", error)
      Swal.fire({
        icon: "error",
        title: "Error",
        text: error.response?.data?.error || "Failed to set weekend off days",
      })
    }
  }

  // Add this function to handle the deletion of off days
  const handleDeleteOffDays = async () => {
    try {
      ...
      const result = await Swal.fire({
        ...
        text: "Are you sure you want to remove all off day schedules for the selected employees?",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#dc3545",
        cancelButtonColor: "#6c757d",
      ...
      if (result.isConfirmed) {
        ...
        if (scheduleTypes.thisMonth) {
          const daysInMonth = currentDate.daysInMonth()
          for (let i = 1; i <= daysInMonth; i++) {
            selectedDays.push(moment(startOfMonth).add(i - 1, "days").format("YYYY-MM-DD"))
          }
        }

        // If no specific period is selected, default to all weekend days in current month
        if (selectedDays.length === 0) {
          const currentDay = moment(startOfMonth)
          while (currentDay.isSameOrBefore(endOfMonth)) {
            // Check if day is Saturday (6) or Sunday (0)
        ...
        if (uniqueSelectedDays.length === 0) {
          Swal.fire({
            icon: "error",
            title: "Error",
            text: "No days selected for removal",
          })
          return
        }

        // Make the API call to delete off day schedules (schedule_type_id = 0)
        const response = await axios.delete(
          `${config.API_BASE_URL}/shift_schedules/delete_shift_schedule_multiple_day/0`,
          {
            headers: {
        ...
        await Swal.fire({
          icon: "success",
          title: "Success",
          text: `Off days have been removed for the selected period`,
          timer: 2000,
          showConfirmButton: false,
        })
      ...
      console.error("Error removing off days:", error)
      Swal.fire({
        icon: "error",
        title: "Error",
        text: error.response?.data?.error || "Failed to remove off days",
      })
    }
  }
              ...
              <div className="card shadow-sm">
                <div className="card-header bg-white d-flex justify-content-between align-items-center">
                  ...
                  <div className="d-flex gap-2">
                    <button
                      className={`btn btn-sm ${selectedEmployees.length === 0 ? "btn-outline-primary" : "btn-primary"}`}
                    ...
                    <span className="badge bg-primary rounded-pill align-self-center">{selectedEmployees.length}</span>
                  </div>
                </div>
                    ...
                    >
                      ...
                      <ul className="list-group list-group-flush">
                        {filteredEmployees.map((emp) => (
                          <li
                            className={`list-group-item list-group-item-action border-0 rounded ${selectedEmployees.includes(emp.emp_ID) ? "bg-light" : ""}`}
                            key={emp.emp_ID}
                  ...
                  {selectedEmployees.length > 0 && (
                    <div className="alert alert-light border mt-3 mb-0 py-2">
                      <div className="d-flex justify-content-between align-items-center">
                        ...
                        <button className="btn btn-sm btn-outline-danger" onClick={() => setSelectedEmployees([])}>
                          Clear
                        </button>
                      </div>
                    </div>
              ...
              <div className="card shadow-sm">
                ...
                <div className="card-header bg-white">
                  <div className="d-flex flex-column flex-lg-row justify-content-between align-items-stretch align-items-lg-center gap-3 mb-3">
                    ...
                    <div className="d-flex flex-wrap gap-2 action-buttons">
                      ...
                      <button
                        className="btn btn-secondary px-3 py-2 d-flex align-items-center"
                        onClick={handleAutoOffWeekends}
                        disabled={selectedEmployees.length === 0}
                        title="Set weekends as off days for selected employees"
                      >
                        <i className="bi bi-calendar2-x-fill me-2 fs-5"></i>
                        <div>
                          <span className="d-block fw-medium">Auto Off</span>
                          <small className="d-block text-white-50">Weekend days</small>
                        </div>
                      </button>

                      {/* New button for removing off days */}
                      <button
                        className="btn btn-outline-secondary px-3 py-2 d-flex align-items-center"
                        onClick={handleDeleteOffDays}
                        disabled={selectedEmployees.length === 0}
                        title="Remove off days for selected employees"
                      >
                        <i className="bi bi-calendar-x me-2 fs-5"></i>
                        <div>
                          <span className="d-block fw-medium">Remove Off</span>
                          <small className="d-block text-muted">Delete off days</small>
                        </div>
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="d-flex justify-content-between align-items-center mb-2">
                    ...
                    <h6 className="mb-0 fs-5 fw-bold text-primary">{currentDate.format("MMMM YYYY")}</h6>
                    <button
                      className="btn btn-sm btn-outline-secondary d-flex align-items-center"
                ...
                <div className="card-body p-2">
                  ...
                  <div className="d-flex gap-3 mb-2 flex-wrap justify-content-end">
                    ...
                    <div className="d-flex align-items-center">
                      <div
                        className="bg-secondary bg-opacity-50 rounded me-1"
                        style={{ width: "12px", height: "12px" }}
                      ></div>
                      <small className="text-muted">Off Days</small>
                    </div>
                  </div>
                              ...
                              return (
                                <td
                                  ...
                                  className={`
                                    ...
                                    ${day && getSchedulesForDay(day).offs.length > 0 ? "bg-secondary bg-opacity-50" : ""}
                                  `}
                                  style={{
                                    height: "110px",
                                    verticalAlign: "top",
                                ...
                                >
                                  {day && (
                                    <>
                                      <div className={`calendar-date ${isToday ? "current-date" : ""}`}>{day}</div>

                                      <div
                                        className="schedule-container"
                                        style={{ fontSize: "0.75rem", marginTop: "20px" }}
                                      >
                                          ...
                                          >
                                            <div className="d-flex justify-content-between align-items-center">
                                              <div className="text-truncate" style={{ maxWidth: "75px" }}>
                                                <div>
                                                  {employees.find((emp) => emp.emp_ID === schedule.emp_ID)?.fName ||
                                                    schedule.emp_ID}
                                                </div>
                                                <small>
                                        ...
                                        {getSchedulesForDay(day).offs.map((schedule, idx) => (
                                          <div
                                            key={`off-${idx}`}
                                            className="schedule-item bg-secondary text-white p-1 mb-1 rounded"
                                            style={{ fontSize: '0.7rem', opacity: 0.8 }}
                                            title={`Off Day: ${employees.find(emp => emp.emp_ID === schedule.emp_ID)?.fName || 'Employee'}`}
                                          >
                                            <div className="d-flex justify-content-between align-items-center">
                                              <div className="text-truncate" style={{ maxWidth: '75px' }}>
                                                <div>{employees.find((emp) => emp.emp_ID === schedule.emp_ID)?.fName || schedule.emp_ID}</div>
                                                <small>
                                                  <i className="bi bi-calendar-x me-1"></i> Off Day
                                                </small>
                                              </div>
                                              <button
                                                className="btn btn-link text-white p-0 ms-1"
                                                onClick={(e) => {
                                                  ...
                                                  handleDeleteSchedule(
                                                    schedule.emp_ID,
                                                    moment(schedule.day).format("YYYY-MM-DD"),
                                                    0, // Schedule type 0 for off days
                                                    false,
                                                  )
                                                }}
            ...
            <div className="modal-dialog modal-dialog-centered">
              <div className="modal-content">
                ...
                <div className="modal-body">
                  <div className="alert alert-light border mb-3">
                    <div className="d-flex align-items-center mb-1">
                      ...
                      <small className="text-muted">
                        Setting regular schedule for <strong>{selectedEmployees.length}</strong> selected employees
                      </small>
                    </div>
                      ...
                      {selectedDates.length > 0 && (
                        <div className="mt-2 p-2 border rounded bg-light">
                          <div className="d-flex justify-content-between align-items-center mb-2">
                            ...
                            <button className="btn btn-sm btn-outline-danger" onClick={() => setSelectedDates([])}>
                              Clear All
                            </button>
                          </div>
                      ...
                      <div className="col">
                        ...
                        <div className="form-check">
                          ...
                          <label className="form-check-label" htmlFor="autoOffWeekend">
                            <i className="bi bi-calendar2-x-fill text-primary me-2"></i>
                ...
                <div className="modal-footer">
                  <button type="button" className="btn btn-secondary" onClick={() => setShowRegularModal(false)}>
                    Cancel
                  </button>
                  <button
                    type="button"
                ...
                <div className="modal-body">
                  <div className="alert alert-light border mb-3">
                    <div className="d-flex align-items-center mb-1">
                      ...
                      <small className="text-muted">
                        Setting overtime schedule for <strong>{selectedEmployees.length}</strong> selected employees
                      </small>
                    </div>
                    <small className="text-muted">Current date: {currentDate.format("MMMM D, YYYY")}</small>
                  </div>
                        ...
                        <div className="mt-2 p-2 border rounded bg-light">
                          ...
                          <div className="d-flex flex-wrap gap-1">
                            {selectedDates.map((date, index) => (
                              <div
                                key={index}
                                className="badge bg-warning text-dark d-flex align-items-center px-2 py-1"
                              >
                                {moment(date).format("MMM DD, YYYY")}
                                <button
                                  type="button"
            ...
            <div className="modal-dialog">
              <div className="modal-content">
                ...
                <div className="modal-footer">
                  ...
                  <button type="button" className="btn btn-danger" onClick={handleRemoveSubmit}>
                    Remove Schedule
                  </button>
                </div>
              </div>
                ...
                <div className="modal-body">
                  ...
                  <div className="alert alert-light border mb-3">
                    <div className="d-flex align-items-center mb-1">
                      ...
                      <small className="text-muted">
                        Removing schedules for <strong>{selectedEmployees.length}</strong> selected employees
                      </small>
                    </div>
                    <small className="text-muted">Current date: {currentDate.format("MMMM D, YYYY")}</small>
                  </div>
                      ...
                      <div className="row row-cols-1 row-cols-md-2 g-3">
                        <div className="col">
                          <div className="form-check mb-3">
                            ...
                            <div className="form-text small ms-4">{currentDate.format("MMM DD, YYYY (dddd)")}</div>
                          </div>
                        </div>
                        ...
                        <div className="col">
                          <div className="form-check mb-3">
                            ...
                            <div className="form-text small ms-4">
                              {moment(currentDate).startOf("week").add(1, "days").format("MMM DD")} -{" "}
                              {moment(currentDate).startOf("week").add(5, "days").format("MMM DD")}
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="col">
                          <div className="form-check">
                            ...
                            <div className="form-text small ms-4">
                              {moment(currentDate).startOf("week").format("MMM DD")} (Sun) &{" "}
                              {moment(currentDate).startOf("week").add(6, "days").format("MMM DD")} (Sat)
                            </div>
                          </div>
                        </div>
                      </div>
          ...
          <div className="modal show d-block" tabIndex="-1">
            <div className="modal-dialog modal-dialog-centered">
              <div className="modal-content">
                ...
                <div className="modal-body">
                  <div className="alert alert-light border mb-3">
                    <div className="d-flex align-items-center mb-1">
                      ...
                      <small className="text-muted">
                        Setting breaks for <strong>{selectedEmployees.length}</strong> selected employees
                      </small>
                    </div>
                    <small className="text-muted">Current date: {currentDate.format("MMMM D, YYYY")}</small>
                  </div>
                ...
                <div className="modal-footer">
                  <button type="button" className="btn btn-secondary" onClick={() => setShowBreaksModal(false)}>
                    Cancel
                  </button>
                  <button type="button" className="btn btn-primary" onClick={handleBreakSubmit}>
            ...
            <div className="modal-dialog modal-dialog-centered">
              <div className="modal-content">
                <div className="modal-header bg-secondary text-white">
                  <h5 className="modal-title">
                    <i className="bi bi-calendar2-x-fill me-2"></i>
                    Auto Off (Weekends)
                  </h5>
                  <button type="button" className="btn-close" onClick={() => setShowAutoOffModal(false)}></button>
                </div>
                <div className="modal-body">
                  <div className="alert alert-light border mb-3">
                    <div className="d-flex align-items-center mb-1">
                      ...
                      <small className="text-muted">
                        Setting weekend days off for <strong>{selectedEmployees.length}</strong> selected employees
                      </small>
                    </div>
                    <small className="text-muted">This will mark Saturdays and Sundays as off days</small>
                  </div>
                  ...
                  <div className="card shadow-sm mb-4 border-secondary">
                    ...
                    <div className="card-body">
                      <div className="mb-3">
                        <div className="form-check mb-2">
                          <input
                            ...
                            name="autoOffPeriod"
                            value="thisMonth"
                            checked={autoOffPeriod === "thisMonth"}
                            onChange={() => setAutoOffPeriod("thisMonth")}
                          />
                          <label className="form-check-label" htmlFor="thisMonth">
                            <i className="bi bi-calendar-month text-secondary me-2"></i>
                        ...
                        <div className="form-check mb-2">
                          <input
                            ...
                            name="autoOffPeriod"
                            value="nextMonth"
                            checked={autoOffPeriod === "nextMonth"}
                            onChange={() => setAutoOffPeriod("nextMonth")}
                          />
                          <label className="form-check-label" htmlFor="nextMonth">
                            <i className="bi bi-calendar-month text-secondary me-2"></i>
                            Next Month ({moment().add(1, "month").format("MMMM YYYY")})
                          </label>
                        </div>
                        ...
                        <div className="form-check mb-3">
                          <input
                            ...
                            name="autoOffPeriod"
                            value="custom"
                            checked={autoOffPeriod === "custom"}
                            onChange={() => setAutoOffPeriod("custom")}
                          />
                          <label className="form-check-label" htmlFor="customPeriod">
                            <i className="bi bi-calendar-range text-secondary me-2"></i>
                            Custom Period
                          </label>
                        </div>

                        {autoOffPeriod === "custom" && (
                          <div className="card border ms-4 p-3 mb-2">
                              ...
                              <div className="col-md-6">
                                <div className="mb-3 mb-md-0">
                                  ...
                                  <input
                                    type="month"
                                    className="form-control"
                                    value={autoOffStartMonth}
                                    onChange={(e) => setAutoOffStartMonth(e.target.value)}
                                  />
                                </div>
                              </div>
                              <div className="col-md-6">
                                <div>
                                  ...
                                  <input
                                    type="month"
                                    className="form-control"
                                    value={autoOffEndMonth}
                                    onChange={(e) => setAutoOffEndMonth(e.target.value)}
                                  />
                                </div>
                              </div>
                            </div>
                            ...
                            {/* Add specific date selection within custom period */}
                            <div className="mt-2">
                              <label className="form-label d-flex align-items-center">
                                <i className="bi bi-calendar-date text-secondary me-2"></i>
                                Add Specific Weekend Dates
                              </label>
                              <div className="input-group">
                                <input
                                  type="date"
                                  className="form-control"
                                  id="customOffDateInput"
                                  onChange={(e) => {
                                    const newDate = e.target.value
                                    // Add to customOffDates if not already there
                                    if (newDate && !customOffDates.includes(newDate)) {
                                      setCustomOffDates([...customOffDates, newDate])
                                    }
                                    // Clear the input after adding
                                    e.target.value = ""
                                  }}
                                ...
                                <button
                                  ...
                                  type="button"
                                  onClick={() => document.getElementById("customOffDateInput").click()}
                                >
                                  <i className="bi bi-plus-lg"></i> Add Date
                                </button>
                              </div>

                              {/* Display selected custom dates */}
                              {customOffDates.length > 0 && (
                                <div className="mt-2 p-2 border rounded bg-light">
                                  <div className="d-flex justify-content-between align-items-center mb-2">
                                    <small className="text-muted">Selected dates ({customOffDates.length})</small>
                                    <button
                                      className="btn btn-sm btn-outline-danger"
                                      onClick={() => setCustomOffDates([])}
                                    >
                                      Clear All
                                    </button>
                                  </div>
                                  <div className="d-flex flex-wrap gap-1">
                                    {customOffDates.map((date, index) => (
                                      <div
                                        key={index}
                                        className="badge bg-secondary d-flex align-items-center px-2 py-1"
                                      >
                                        {moment(date).format("MMM DD, YYYY")}
                                        <button
                                          type="button"
                                          ...
                                          style={{ fontSize: "0.5rem" }}
                                          onClick={() => {
                                            const newDates = [...customOffDates]
                                            newDates.splice(index, 1)
                                            setCustomOffDates(newDates)
                                          }}
                                        ></button>
                                      </div>
                                    ))}
                              ...
                              <div className="form-text mt-2">
                                <i className="bi bi-info-circle me-1"></i>
                                Select specific dates if you need to mark additional days as off days
                              </div>
                            </div>
                          </div>
                        )}
                      ...
                      <div className="alert alert-info d-flex align-items-center">
                        ...
                        <div>
                          <strong>Weekend days</strong> will be automatically marked as off days.
                          <div className="small">This simplifies scheduling for regular work weeks.</div>
                        </div>
                      </div>
                    </div>
                  </div>
                  {/* Add to the Auto Off modal, inside the card before the alert */}
                  <div className="card shadow-sm mb-3 border-secondary">
                    <div className="card-header bg-secondary bg-opacity-10">
                      <h6 className="mb-0">Off-Day Time Configuration</h6>
                    </div>
                    <div className="card-body">
                      <div className="row">
                        <div className="col-md-6">
                          <div className="mb-3">
                            <label htmlFor="offDayShiftIn" className="form-label d-flex align-items-center">
                              <i className="bi bi-clock me-2 text-muted"></i>
                              Shift In Time
                            </label>
                            <input
                              ...
                              onChange={(e) => setShiftInTime(e.target.value)}
                            />
                            <div className="form-text small">Default "00:00" for typical off days</div>
                          </div>
                        </div>
                        <div className="col-md-6">
                          <div className="mb-3">
                            <label htmlFor="offDayShiftOut" className="form-label d-flex align-items-center">
                              <i className="bi bi-clock-history me-2 text-muted"></i>
                              Shift Out Time
                            </label>
                            <input
                      ...
                      <div className="text-end">
                        <button
                          ...
                          onClick={() => {
                            setShiftInTime("00:00")
                            setShiftOutTime("00:00")
                            Swal.fire({
                              toast: true,
                              position: "top-end",
                              showConfirmButton: false,
                              timer: 1500,
                              icon: "info",
                              title: "Reset to default off day times",
                            })
                          }}
                        >
                          <i className="bi bi-arrow-counterclockwise me-1"></i> Reset to Default
                ...
                <div className="modal-footer">
                  <button type="button" className="btn btn-secondary" onClick={() => setShowAutoOffModal(false)}>
                    Cancel
                  </button>
                  <button type="button" className="btn btn-primary" onClick={handleAutoOffSubmit}>
                    <i className="bi bi-save me-1"></i> Set Weekend Days Off
                  </button>
                </div>
              </div>
            </div>
